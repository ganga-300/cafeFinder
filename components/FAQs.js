...
export default function FAQs() {
  ...
  return (
    <div className="py-16 px-4 md:px-10 bg-orange-100 font-sans">
      <div className="container mx-auto max-w-4xl">
        <h2 className="text-3xl text-amber-900 text-center mb-10 font-playfair font-bold">Frequently Asked Questions</h2>
          ...
          {faqData.map((item, index) => (
            ...
            >
              <div className="flex justify-between items-center">
                <span className="text-lg font-semibold text-gray-800 font-poppins">{item.question}</span>
                <span className={`text-xs transition-transform duration-300 text-amber-900 ${activeIndex === index ? 'rotate-180' : ''}`}>
                  ▼
                </span>
              </div>
              <div className={`text-base text-gray-600 mt-3 transition-all duration-300 font-poppins ${activeIndex === index ? 'max-h-52 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                {item.answer}
              </div>
            </div>
          ))}
